// Assignment 3
// By: Paul Kafka
// This class uses my Linklist class to 
...
#include<cstdlib>
#include<algorithm>
#include "file.h"   // This file contains my "Linklist.h" class
					// where the words are stored in a List like the
	...
	for (words_itr=words.begin(); words_itr!=words.end(); ++words_itr){			//loop through words
			for (itr3=s2.begin(); itr3 !=s2.end(); ++itr3){					// loop through the word list
				if (*words_itr == *itr3)	{							// If a word matches
					...
					words.Self_Organinize(words_itr,  *words_itr);	// move that word two steps to the front
				}	// SELF ORGANIZATION!!!
			}
		}
	}
